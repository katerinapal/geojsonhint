...
test('geojsonhint', function(t) {
    ext_glob_glob.sync('test/data/good/*.geojson').forEach(function(f) {
        var gj = file(f);
        t.deepEqual(index_hintjs(gj), [], f);
    });
    t.deepEqual(index_hintjs(undefined), [{
        message: 'Expected string or object as input',
        line: 0
    }], 'expected string input');
    t.deepEqual(index_hintjs('{}'), [{
    ...
    test('validates incorrect files', function(t) {
        ext_glob_glob.sync('test/data/bad/*.geojson').forEach(function(f) {
            ...
            if (process.env.UPDATE) {
                ext_fs_fs.writeFileSync(f.replace('geojson', 'result'), JSON.stringify(index_hintjs(gj), null, 2));
            }
            t.deepEqual(index_hintjs(gj), filejs(f.replace('geojson', 'result')), f);
        });
        t.end();
    });
    ...
    test('validates incorrect files as objects', function(t) {
        ext_glob_glob.sync('test/data/bad/*.geojson').forEach(function(f) {
            ...
            var gj = filejs(f);
            if (process.env.UPDATE) {
                ext_fs_fs.writeFileSync(f.replace('geojson', 'result-object'), JSON.stringify(index_hintjs(gj), null, 2));
            }
            t.deepEqual(index_hintjs(gj), filejs(f.replace('geojson', 'result-object')), f);
        });
        t.end();
    });
    test('noDuplicateMembers option=false', function(t) {
        t.deepEqual(index_hintjs('{"type":"invalid","type":"Feature","properties":{},"geometry":null}', {
    ...
    test('noDuplicateMembers option=true', function(t) {
        t.deepEqual(index_hintjs('{ "type": "Point", "coordinates": [100.0000000001, 5.0000000001] }', {
    ...
    test('noDuplicateMembers option=true', function(t) {
        ...
        var truncated = index_hintjs(JSON.stringify(featureCollection, null, 2), {
            precisionWarning: true
        })
    ...
    test('invalid roots', function(t) {
        t.deepEqual(index_hintjs('null'), [{
            message: 'The root of a GeoJSON object must be an object.',
            line: 0
        }], 'non-object root');
        t.deepEqual(index_hintjs('1'), [{
            message: 'The root of a GeoJSON object must be an object.',
            line: 0
        }], 'number root');
        t.deepEqual(index_hintjs('"string"'), [{
            message: 'The root of a GeoJSON object must be an object.',
            line: 0
        }], 'string root');
        t.end();
