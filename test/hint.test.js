...
test('geojsonhint', function(t) {
    ext_glob_glob.sync('test/data/good/*.geojson').forEach(function(f) {
        var gj = file(f);
        t.deepEqual(hint(gj), [], f);
    });
    t.deepEqual(hint(undefined), [{
    ...
    test('validates incorrect files', function(t) {
        ext_glob_glob.sync('test/data/bad/*.geojson').forEach(function(f) {
            ...
            if (process.env.UPDATE) {
                ext_fs_fs.writeFileSync(f.replace('geojson', 'result'), JSON.stringify(hint(gj), null, 2));
            }
            t.deepEqual(hint(gj), filejs(f.replace('geojson', 'result')), f);
        });
        t.end();
    });
    ...
    test('validates incorrect files as objects', function(t) {
        ext_glob_glob.sync('test/data/bad/*.geojson').forEach(function(f) {
            ...
            var gj = filejs(f);
            if (process.env.UPDATE) {
                ext_fs_fs.writeFileSync(f.replace('geojson', 'result-object'), JSON.stringify(hint(gj), null, 2));
            }
            t.deepEqual(hint(gj), filejs(f.replace('geojson', 'result-object')), f);
        });
        t.end();
    });
    test('noDuplicateMembers option=false', function(t) {
        t.deepEqual(hint('{"type":"invalid","type":"Feature","properties":{},"geometry":null}', {
    ...
    test('noDuplicateMembers option=true', function(t) {
        t.deepEqual(hint('{ "type": "Point", "coordinates": [100.0000000001, 5.0000000001] }', {
    ...
    test('noDuplicateMembers option=true', function(t) {
        ...
        var truncated = hint(JSON.stringify(featureCollection, null, 2), {
            precisionWarning: true
        })
